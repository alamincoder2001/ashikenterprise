...
class Products extends CI_Controller {
    ...
    public function image_upload($file_name_get){
		...
		move_uploaded_file($file_temp,'uploads/products/'.$new_name);
		return $new_name;
	}
    ...
    public function addProduct() {
        ...
        try{
            ...
            $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and status = 'a'", $productObj->Product_Name)->num_rows();
            ...
            $product['image'] = $this->image_upload($_FILES['image']);
            $product['status'] = 'a';
    ...
    public function updateProduct(){
        ...
        try{
            ...
            // image update 
            $image = '';
            $oldImage = $this->db->query("select * from tbl_product where Product_SlNo = ?
            ", $productObj->Product_SlNo)->row();
            if(isset($_FILES['image'])){
                $image = $this->image_upload($_FILES['image']);
                if($oldImage->image != null)
                {
                    $img_unlink = 'uploads/products/'.$oldImage->image;
                    unlink($img_unlink);
                }
            } else {
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(sr.quantity), 0) 
                    from tbl_shipping_return sr
                    join tbl_salesmaster sm on sm.SaleMaster_SlNo = sr.sale_id
                    where sr.product_id = p.Product_SlNo
    ...
    public function getProductLedger(){
        ...
        $result = $this->db->query("
            ...
            select
                ...
                sr.id,
                sm.SaleMaster_SaleDate as date,
                'Shipping Return' as description,
                0 as rate,
                0 as in_quantity,
                0 as out_quantity,
                sr.quantity return_quantity
            from tbl_shipping_return sr
            join tbl_salesmaster sm on sm.SaleMaster_SlNo = sr.sale_id
            where sr.product_id = " . $data->productId . "
            and sr.branch_id = " . $this->brunch . "

            order by date, sequence, id
        ")->result();

        $ledger = array_map(function($key, $row) use ($result){
            $row->stock = $key == 0 ? ($row->in_quantity + $row->return_quantity) - $row->out_quantity : ($result[$key - 1]->stock + (($row->in_quantity + $row->return_quantity) - $row->out_quantity));
            return $row;
