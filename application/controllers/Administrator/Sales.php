...
class Sales extends CI_Controller
{
    ...
    public function addSales()
    {
        ...
        try {
            ...
            if (isset($data->customer)) {
                ...
                if ($data->customer->Customer_Mobile != '' && $data->customer->Customer_Mobile != null && $mobile_count->num_rows() > 0) {
                    $duplicateCustomer = $mobile_count->row();
                    unset($customer['Customer_Code']);
                    unset($customer['District_Name']);
                    unset($customer['Employee_Name']);
                ...
                } else if (isset($data->customer) && $data->customer->Customer_Type == 'N') {
                    $customer = (array)$data->customer;
                    unset($customer['Customer_SlNo']);
                    unset($customer['display_name']);
                    unset($customer['Customer_Type']);
        ...
        $saleDetails = $this->db->query("
            select 
                ...
                (
                    ...
                    where sr.sale_id = sd.SaleMaster_IDNo
    ...
    public function getSalesRecord()
    {
        ...
        if (isset($data->employeeId) && $data->employeeId != '') {
            ...
            $empdate = "";
            if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
                $empdate = " AND r.returnDate between '$data->dateFrom' AND '$data->dateTo'";
            }

            $empReturn = $this->db->query("SELECT
                                    p.Product_Code,
                                    p.Product_Name,
                                    p.Product_Purchase_Rate,
                                    p.Product_SellingPrice,
                                    r.returnDate,
                                    sum(r.returnQty) as quantity,
                                    sum(r.totalAmount) as total,
                                    r.Status,
                                    r.branchId
                                FROM tbl_empwise_productreturn r
                                LEFT JOIN tbl_product p ON p.Product_SlNo = r.productId
                                WHERE r.Status = 'a'
                                AND r.branchId = '$this->sbrunch'
                                AND r.Employee_Id = '$data->employeeId'
                                $empdate
                                GROUP BY p.Product_Code
                                ")->result();

            $res['empReturn'] = $empReturn;
        }
        ...
        foreach ($sales as $sale) {
            $sale->saleDetails = $this->db->query("
                select
                    ...
                    (select sd.SaleDetails_TotalQuantity - returnQty) as Quantity,
                    (select sd.SaleDetails_Rate * Quantity) as saleAmount,
                    (select sd.Purchase_Rate * Quantity) as purchased_amount,
                    (select saleAmount - purchased_amount) as profit_loss
                from tbl_saledetails sd 
                join tbl_product p on p.Product_SlNo = sd.Product_IDNo
                where sd.SaleMaster_IDNo = ?
            ", $sale->SaleMaster_SlNo)->result();
