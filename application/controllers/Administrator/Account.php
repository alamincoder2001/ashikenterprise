    ...
    public function getCashLedger() {
        ...
        $ledger = $this->db->query("
            ...
            select 
                dm.id,
                dm.date,
                concat('Damage Claim ') as description,
                ( select ifnull(sum(sd.total), 0) 
                    from tbl_shipping_damage sd 
                    where sd.claim_id = dm.id
                    and sd.status = 'c'
                ) as in_amount,
                0.00 as out_amount
            from tbl_damage_claim dm
            where dm.branch_id= '$this->brunch'
            and dm.status = 'c'
            and dm.date between '$data->fromDate' and '$data->toDate'
