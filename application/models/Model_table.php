...
class Model_Table extends CI_Model{
    ...
    public function getTransactionSummary($date = null){
        $transactionSummary = $this->db->query("
            ...
            (
                select ifnull(sum(sd.total), 0) from tbl_shipping_damage sd
                where sd.branch_id = " . $this->session->userdata('BRANCHid') . "
                and sd.status = 'c'
                " . ($date == null ? "" : " and sd.date < '$date'") . "
            ) as damage_claim,
            ...
            /* total */
            (
                select received_sales + received_customer + received_supplier + received_cash + bank_withdraw + loan_received + loan_initial_balance + invest_received + sale_asset + damage_claim
            ) as total_in,
            (
    ...
    public function customerDue($clauses = "" , $date = null) {
        ...
        $dueResult = $this->db->query("
            ...
            (select invoicePaid + cashReceived + discountAmount) as paidAmount,
    ...
    public function productStock($productId) {
        ...
        if($stockCount != 0){
            ...
            $stock = ($stockRow->purchase_quantity + $stockRow->transfer_to_quantity + $stockRow->sales_return_quantity + $stockRow->empwise_return_quantity) 
