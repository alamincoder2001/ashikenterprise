				...
				<div class="col-xs-6">
					<table class="balancesheet-table">
						...
						<tbody>
							...
							<tr>
								<td>Damage Claim</td>
								<td style="text-align:right;">{{ totalClaimDamage | decimal }}</td>
							</tr>
						</tbody>
						<tfoot>
							<tr>
...
<script>
	new Vue({
		...
		computed: {
			...
			totalClaimDamage() {
				return this.damages.reduce((prev, curr) => {
			...
			totalCashIn() {
				return parseFloat(this.totalSales) +
					...
					parseFloat(this.totalClaimDamage);
			},
			totalCashOut() {
		...
		methods: {
			...
			async getClaimDamage() {
				let filter = {
					dateFrom: this.filter.dateFrom,
					dateTo: this.filter.dateTo,
				}
				await axios.post('/get_claim_damage', filter).then(res => {
