				...
				>
					<table class="record-table" v-if="selectedProduct != null">
						...
						<tbody>
							<tr v-for="sale in sales">
								...
								<td style="text-align:right;">{{ sale.SaleDetails_TotalQuantity - sale.returnQty }}</td>
							</tr>
						</tbody>
						<tfoot>
							<tr style="font-weight:bold;">
								<td colspan="5" style="text-align:right;">Total Quantity</td>
								<td style="text-align:right;">{{ sales.reduce((prev, curr) => { return prev + parseFloat(curr.SaleDetails_TotalQuantity - curr.returnQty)}, 0) }}</td>
							</tr>
						</tfoot>
					</table>
					...
					<table class="record-table" v-if="selectedProduct == null">
						...
						<tbody>
							<template v-for="sale in sales">
								<tr>
									<td colspan="6" style="text-align:center;background: #ccc;">{{ sale.category_name }}</td>
								</tr>
								<tr v-for="product in sale.products">
									<td>{{ product.product_code }}</td>
									<td>{{ product.product_name }}</td>
									<td style="text-align:right;">{{ product.quantity }}</td>
									<td style="text-align:right;">{{ product.r_quantity }}</td>
									<td style="text-align:right;">{{ product.s_quantity }}</td>
								</tr>
							</template>
						</tbody>
					</table>
